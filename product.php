...
if (isset($_POST['add'])) {
    ...
    $price_of_purchase = $_POST['product-price-of-purchase'];
    $sale_price = $_POST['product-sale-price'];
    $brands = $_POST['product-brands'];
    ...
    if (move_uploaded_file($tmp_name, $upload_path)) {
        $stmt = $conn->prepare("INSERT INTO `product`(`name`, `category`, `quantity`, `price_of_purchase`, `sale_price`, `brands`, `description`, `picture`) VALUES (:name, :category, :quantity, :price_of_purchase, :sale_price, :brands, :description, :picture)");
        $stmt->bindParam(':name', $name);
        $stmt->bindParam(':category', $category);
        $stmt->bindParam(':quantity', $quantity);
        $stmt->bindParam(':price_of_purchase', $price_of_purchase);
...
if (isset($_POST['edit'])) {
    ...
    $new_price_of_purchase = $_POST['product-price-of-purchase'];
    $new_sale_price = $_POST['product-sale-price'];
    $new_brands = $_POST['product-brands'];
    ...
    $stmt = $conn->prepare("UPDATE `product` SET `name`=?, `category`=?, `quantity`=?, `price_of_purchase`=?, `sale_price`=?, `brands`=?, `description`=? WHERE `id`=?");
    $stmt->execute([$new_name, $new_category, $new_quantity, $new_price_of_purchase, $new_sale_price, $new_brands, $new_description, $product_id]);
