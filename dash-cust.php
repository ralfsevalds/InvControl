...
<script>
// Funkcija klienta informācijas iegūšanai un atjaunināšanai
function updateCustomerDetails(customerId) {
  // Iegūstiet klienta informāciju no servera
  fetch('get_customer_details.php', {
    ...
    .then(customerData => {
      // Atjauniniet klienta informāciju lapā
      ...
      // Iestatiet klienta informāciju, pamatojoties uz iegūtajiem datiem
...
// Notikumu klausītājs, lai skatītu detalizētu informāciju
document.querySelectorAll('.view-details-btn').forEach(function(button) {
  button.addEventListener('click', function() {
    // Izgūstiet klienta ID no noklikšķinātās rindas
    var customerId = this.closest('tr').querySelector('td:first-child').textContent;

    // Zvaniet uz funkciju, lai atjauninātu klienta informāciju
    updateCustomerDetails(customerId);

    // Ievades laukos iestatiet klienta informāciju
